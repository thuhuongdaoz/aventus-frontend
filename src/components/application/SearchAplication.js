...
export default function SearchApplication() {
    ...
    return (
        ...
        <div className='list-job'>
            <h1 className='list-job-title'>Tìm kiếm ứng viên</h1>
                    ...
                    {applications.slice((currentPage - 1) * pageSize, ((currentPage - 1) * pageSize) + pageSize).map(application => (
                        <Link to={`/viewapplication/${application.id}`} key={application.id} className='job pa-4 mb-5 flex gap-8 pa-3 mb-3'>
                            ...
                            <div className='flex flex-col justify-between w-full'>
                                ...
                                <div>
                                    {/* <Tag color="cyan" >{application.major.name}</Tag> */}
                                    ...
                                    {/* <Tag color="cyan" >{application.status == 1 ? 'Chờ duyệt' : (application.status == 2 ? 'Chấp nhận' : 'Từ chối')}</Tag> */}
                                </div>
                            </div>
                        </Link>
                    ))}
